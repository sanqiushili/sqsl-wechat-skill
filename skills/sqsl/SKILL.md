...
name: sqsl
description: |
  SQSL (三秋十李) 微信公众号创作与排版家族总路由。
  双模式智能调度：
  1. 任务前路由：智能识别用户意图（发文章、克隆排版、配置微信API、查看引导），自动流转到对应的专业子技能；
  2. 任务后导航：风格克隆完成后自动引导调用排版直推，排版完成后提供草稿箱直推与多风格切换指引。
  触发方式：/sqsl、/三秋十李、/公众号全家桶、「发公众号」「帮我排版」「克隆公众号排版」「使用引导」「新手帮助」
---

# 🏛️ SQSL (三秋十李) 微信公众号创作与排版家族总路由

欢迎来到 **SQSL 微信公众号高品质创作与排版全家桶**。本技能是整个工具箱的主调度入口，负责识别你的需求并自动协同下属专业子技能。
...
```mermaid
flowchart TD
    User["用户输入 (/sqsl)"] --> Router{"意图识别总路由 (/sqsl)"}
...
当用户唤起 `/sqsl` 或用自然语言提问时，按以下逻辑进行精准意图分流：
...
> 
> 1. **想排版发布文稿？**
>    - 发送：`/sqsl 帮我排版 docs/my_post.md 并推送到公众号`
>    - 或发送：`/sqsl-article-to-wechat docs/my_post.md --style olive_artisan`
> 2. **看到别人家排版好看，想据为己有？**
>    - 发送：`/sqsl 帮我克隆这篇公众号排版：https://mp.weixin.qq.com/s/...`
